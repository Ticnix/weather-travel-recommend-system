...
async def collect_news(
    ...
    """采集真实气象资讯（需登录），资讯统一本地化为广州内容。

    - area：预警筛选的地区关键词（默认「广州」，本地无预警时自动回退「广东」）
    - with_news：是否采集中国天气网气象新闻（只保留本地相关）
    - with_tavily：是否用 Tavily 联网搜索采集广州本地资讯（需配置 Key）
    - with_forecast：是否生成一份广州未来天气简报
    采集结果按标题去重，可重复调用。
    """
