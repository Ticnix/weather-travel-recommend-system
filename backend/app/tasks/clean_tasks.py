...
def run_clean(task_id: str, filename: str, stored_path: str,
    ...
    # 标记 running
    _run(update_task_stats(task_id, {"status": "running", "log": "任务开始执行"}, use_celery_engine=True))

    try:
        stats = clean_csv(stored_path, cleaned_path)
        _run(update_task_stats(
            task_id,
            {
                "status": "success",
